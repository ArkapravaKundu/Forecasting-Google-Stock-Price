...
# ## Prepare Google training dataset

# In[2]:


# Reading Google index prices csv file
with open('./data/google_data.csv', 'r') as csvfile:
...
# ## Basic Data analysis for Google dataset
...
idx = pd.date_range('1-7-2008', '6-29-2018')
...
for year in years:
    if year==2007:
        break
    for month in months:
        file_str = './data/nytimes/'+str('nytimes') + str(year) + '-' + '{:02}'.format(month) + '.json'
        if file_str=='./data/nytimes/nytimes2018-06.json':
            break
        print(year)
...
#interpolated_df.to_csv('./data/News_df_10_years_filtered_lead_para.csv',sep='\t', encoding='utf-8')
dft.dropna(inplace=True)
dft.to_excel("NyTimes_all.xlsx", index = True, encoding='utf-8')
#interpolated_df.to_csv("submission3.csv")
...
# Saving the data as pickle file
dft.to_pickle('./data/Nytimes_all.pkl') 
